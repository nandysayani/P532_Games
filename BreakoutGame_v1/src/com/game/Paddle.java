...
public class Paddle implements Sprite {
	...
	public void setXPos(int xPos) {
		...
		if (xPos > (Canvas.WIDTH - P_WIDTH)) // ensure there is enough space for
	...
	public boolean hitPaddle(Ball b) {
		if (b.getxPos() <= xPos + (P_WIDTH + 15)) {// Within right edge
			if (b.getxPos() >= xPos - 10) {// Within left edge
				if ((b.getyPos() + (Ball.DIAMETER - 1)) >= (Y_POS)) {// Top of paddle
					System.out.println("Ball Y"+ b.getyPos() + " paddle y: "+Y_POS);
					if ((b.getyPos() + (Ball.DIAMETER - 1)) <= (Y_POS + (P_HEIGHT - 5))) {
						System.out.println("Heere Ball Y"+ b.getyPos() + " paddle y: "+Y_POS);
						return true;
					}
				}
			}
